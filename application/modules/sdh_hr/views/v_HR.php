...
<div class="FE_95 FE_form" id="FAD_UR" style="margin-top:10px;">
	...
	<div class="alert alert-success <?php echo $class_dml; ?>" id="Was_TR">
		<?php //<button type="button" class="close" data-dismiss="">&times;</button>?>
...
<div class="FE_95 FE_border FE_form FE_datatable" id="DATA_OP_SDH" style="margin-top:10px;">
	<div class="FE_100 FE_datatable">
		<div class="FE_100 FE_datatable">
			<h4>Hold Request (was hold by You)</h4>
		</div>
		<table style="font-size:11px;" class="datatable">
			...
			<?php
			$N = 1;
			foreach ($SR_HR_SDH_DATA as $HR)
			{
				$REQID_WT_SLASH = str_replace('/','_',$HR->request_id);
				$REQID_WT_DASH = str_replace('-','_',$REQID_WT_SLASH);
				?>
				<script type="text/javascript">
					var DR_<?php echo $N; ?> = "<?php echo $HR->request_id; ?>";
				</script>
				<tr>
					<th><?PHP echo$N; ?></th>
						<input type="hidden" id="DATA_WR_UR_<?php echo $N; ?>" value="<?php echo$HR->request_id; ?>">
					<td><?PHP echo$HR->request_id; ?></td>
					<td><?PHP echo$HR->request_based; ?></td>
					<td><?PHP echo$HR->request_type; ?></td>
					<td><?PHP echo$HR->request_case; ?></td>
					<td><?PHP echo$HR->request_modul; ?></td>
					<td><?PHP echo$HR->request_desc; ?></td>
					<td><?PHP echo$HR->request_reason; ?></td>
					<td><?PHP echo$HR->request_status; ?></td>
					<td><?PHP echo $HR->request_dateCreated ; ?></td>
					<th><i class="icon-list ttip" id="BTN_ACT_GR_<?php echo $REQID_WT_DASH; ?>" onClick="openFtrMdl(DR_<?php echo $N; ?>)" data-toggle="tooltip" data-placement="top" title="Detail & Action"></i></th>
				</tr>
				<?php
				$N++;
			}
...
<div id="Modal_FTR" class="modal hide fade">
	...
	<div class="modal-body">
		...
		<table class="FE_modal">
			<tr>
				...
				<td>
					<select name="status" id="status">
						...
						<option value="On Process" <?php echo $choose_OP; ?>>On Process</option>
						<option value="Hold"<?php echo $choose_hold; ?> >Hold</option>
			...
			<tr class="<?php echo $show_EST_timeDone; ?>" id="SET_DONE">
				<td>Set Estimate Time</td>
