...
<div class="FE_95 FE_border FE_form FE_datatable" id="DATA_OP_SDH" style="margin-top:10px;">
	<div class="FE_100 FE_datatable">
		<div class="FE_100 FE_datatable">
			<h4>Canceled Request (Request that canceled by user or PIC)</h4>
		</div>
		<table style="font-size:11px;" class="datatable">
			...
			<?php
			$N = 1;
			foreach ($SR_CR_SDH_DATA as $CR)
			{
				$REQID_WT_SLASH = str_replace('/','_',$CR->request_id);
				$REQID_WT_DASH = str_replace('-','_',$REQID_WT_SLASH);
				?>
				<script type="text/javascript">
					var DR_<?php echo $N; ?> = "<?php echo $CR->request_id; ?>";
				</script>
				<tr>
					<th><?PHP echo$N; ?></th>
						<input type="hidden" id="DATA_WR_UR_<?php echo $N; ?>" value="<?php echo$CR->request_id; ?>">
					<td><?PHP echo$CR->request_id; ?></td>
					<td><?PHP echo$CR->requested_FullName; ?></td>
					<td><?PHP echo$CR->request_based; ?></td>
					<td><?PHP echo$CR->request_type; ?></td>
					<td><?PHP echo$CR->PIC_fullname; ?></td>
					<td><?PHP echo$CR->request_status; ?></td>
					<th><?PHP echo $CR->request_dateCreated ; ?></th>
					<th><i class="ttip icon-list" onClick="openMdl(DR_<?php echo $N; ?>)" data-toggle="tooltip" data-placement="top" title="Detail this request"></i></th>
				</tr>
				<?php
				$N++;
			}
