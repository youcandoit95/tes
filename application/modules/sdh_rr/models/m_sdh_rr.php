...
class M_sdh_rr extends CI_Model
{
	...
	public function flag_read_notif_revision()
	{
		$sess_login_level = $this->session->userdata('user_level');
	...
	public function revision_request($p_req_no,$p_req_est_time,$user_no) // alias op
	{
		$sql = "UPDATE TBL_REQUEST SET 	
						REQ_LSTATUS=4,
						REQ_NOTIF_READ=4,
						REQ_NOTIF_READ_DEV=4,
		...
		if ($query)
		{
			...
			$sql = "INSERT INTO TBL_REQUEST_HISTORY
							(HISTORY_NO, REQ_NO, REQ_STATUS, REQ_EST_TIME, HISTORY_BY)
						VALUES 
							('$p_history_no','$p_req_no',4,TO_DATE('$p_req_est_time','DD-MM-RRRR HH24:MI:SS'),'$user_no')";
			$query = $this->db->Execute($sql);
			if ($query)
			{			
				return true;
