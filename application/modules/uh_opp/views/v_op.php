...
<div class="FE_95 FE_border FE_form FE_datatable" id="DATA_OP_SDH" style="margin-top:10px;">
	<div class="FE_100 FE_datatable">
		<div class="FE_100 FE_datatable">
			<h4>On Process Request (That you requested)</h4>
		</div>
		<table style="font-size:11px;" class="datatable">
			...
			<?php
			$N = 1;
			foreach ($SR_OP_UH_DATA as $OP)
			{
				$REQID_WT_SLASH = str_replace('/','_',$OP->request_id);
				$REQID_WT_DASH = str_replace('-','_',$REQID_WT_SLASH);
				?>
				<script type="text/javascript">
					var DR_<?php echo $N; ?> = "<?php echo $OP->request_id; ?>";
				</script>
				<tr>
					<th><?PHP echo$N; ?></th>
						<input type="hidden" id="DATA_WR_UR_<?php echo $N; ?>" value="<?php echo$OP->request_id; ?>">
					<td><?PHP echo$OP->request_id; ?></td>
					<td><?PHP echo$OP->request_based; ?></td>
					<td><?PHP echo$OP->request_type; ?></td>
					<td><?PHP echo$OP->request_case; ?></td>
					<td><?PHP echo$OP->request_modul; ?></td>
					<td><?PHP echo$OP->request_desc; ?></td>
					<td><?PHP echo$OP->request_reason; ?></td>
					<td><?PHP echo$OP->request_status; ?></td>
					<td><?PHP echo $OP->request_dateCreated ; ?></td>
					<th><i class="icon-list ttip" id="BTN_ACT_GR_<?php echo $REQID_WT_DASH; ?>" onClick="openTR(DR_<?php echo $N; ?>)" data-toggle="tooltip" data-placement="top" title="Detail & Action"></i> <a href="<?php echo $rep1.$rep2.$OP->request_id ?>" target="_blank"><i class="icon-print ttip" data-toggle="tooltip" data-placement="top" title="Print"></i></a></th>
				</tr>
				<?php
				$N++;
			}
...
<div id="Modal_FTR" class="modal hide fade">
	...
	<div class="modal-header">
		...
		<span style="font-size:21px">Update Status Request</span>
	</div>
	<div class="modal-body">
		...
		<table class="FE_modal">
			...
			<tr class="<?php echo $show_hold_reason; ?>" id="STAT_REASON">
				...
				<td><textarea class="form" id="FTR_REQ_Reason" name="FTR_REQ_Reason" cols=10 rows=4></textarea></td>
			</tr>
			<tr class="gone" id="SET_DONE">
				...
				<input type="hidden" name="addr_from" value="on_process">
			<tr>
				<td colspan=2>&nbsp;</td>
			</tr>
	...
	<div class="modal-footer">
		...
		<input type="submit" class="btn btn-success FE_btn_take" value="SET Status!">
	</div>
</div>