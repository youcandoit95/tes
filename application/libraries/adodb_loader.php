...
class Adodb_loader
{
    ...
    function _init_adodb_library($dbh,$db_group)
    {
        ...
        if (isset($cfg['dbdriver']))
        {
            ...
            {
								if($cfg['dbdriver'] == 'oci8')
								{
								
								$CI->$dbh->NLS_DATE_FORMAT =  'DD-MM-RRRR HH24:MI';
               	$cstr = "(DESCRIPTION =
												 (ADDRESS = (PROTOCOL = TCP)(HOST = $cfg[hostname])(PORT = 1521))
												 (CONNECT_DATA = (SERVICE_NAME = orion)))";
								$CI->$dbh->Connect($cfg['hostname'],$cfg['username'],$cfg['password']);
								//$CI->$dbh->Connect($cfg['hostname'], $cfg['username'], $cfg['password'], 'orion');
								}
								else
								{											
								// normal
